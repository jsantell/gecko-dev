...
const TableRenderer = exports.TableRenderer = Class({
  ...
  destroy: function() {
    ...
    requests.off("add", this._onRequestAdd);
    requests.off("change", this._onRequestUpdate);
    requests.off("sort", this._onRequestSort);
    requests.off("filtered", this._onRequestFilter);
    requests.off("reset", this._onRequestReset);
    ...
    while (!optimalTickIntervalFound) {
      ...
      for (let i = 1; i <= REQUESTS_WATERFALL_BACKGROUND_TICKS_SCALES; i++) {
        ...
        for (let x = 0; x < canvasWidth; x += increment) {
          let position = (this.document.defaultView.isRTL ? canvasWidth - x : x) | 0;
