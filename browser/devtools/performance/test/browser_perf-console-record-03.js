...
function* spawnTest() {
  ...
  let rustRec = recordings.find(r => r.getLabel() === "rust");
  let rust2Rec = recordings.find(r => r.getLabel() === "rust2");

  is(rustRec.isConsole(), true, "recording came from console.profile (1).");
  is(rustRec.getLabel(), "rust", "correct label in the recording model (1).");
  is(rustRec.isRecording(), false, "recording is still recording (1).");
  is(rust2Rec.isConsole(), true, "recording came from console.profile (2).");
  is(rust2Rec.getLabel(), "rust2", "correct label in the recording model (2).");
  is(rust2Rec.isRecording(), true, "recording is still recording (2).");
