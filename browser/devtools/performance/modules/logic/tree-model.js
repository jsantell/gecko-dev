...
function FrameNode(frameKey, { location, line, category, allocations, isContent }, isMetaCategory) {
  ...
  // All FrameNodes that come from an inflated frame should already
  // have a category. Use `getCategory` when a FrameNode is not being created
  // via InflatedFrames (only in tests?)
  this.category = category || FrameUtils.getCategory(this);
}
