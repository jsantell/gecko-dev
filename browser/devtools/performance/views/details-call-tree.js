...
function viewSourceInDebugger(url, line) {
  ...
  return gToolbox.selectTool("jsdebugger").then(({ panelWin: dbg }) => {
    if (debuggerAlreadyOpen) {
      return dbg;
    } else {
      return new Promise((resolve) => dbg.once(dbg.EVENTS.SOURCES_ADDED, () => resolve(dbg)));
    }
  }).then(function ({ DebuggerView }) {
