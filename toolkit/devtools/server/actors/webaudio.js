...
let AudioNodeActor = exports.AudioNodeActor = protocol.ActorClass({
  ...
  addAutomationEvent: method(function (paramName, eventName, args=[]) {
    ...
    try {
      ...
      // Using the wrapped node and parameter, the corresponding
      // WebAudioActor event will NOT be fired. This is due to
      // being unable to apply a Float32Array to an unwrapped, content
      // parameter due to not having Float32Array from the same context.
      // This should be ok, as this is only used in tests.
      let param = node[paramName];
      param[eventName].apply(param, args);

      // Record automation data here since the WebAudioActor event will not be called.
      this._recordAutomationEvent(paramName, eventName, args);
    } catch (e) {
      return constructError(e);
    }
  }, {
...
/**
 * Casts TypedArray to a normal array.
 */
function castToArray (a) {
  let newArray = [];
  // The Xray machinery for TypedArrays denies indexed access on the grounds
  // that it's slow, and advises callers to do a structured clone instead.
  let global = Cu.getGlobalForObject(this);
  let safeView = Cu.cloneInto(a.subarray(), global);
  for (let i = 0; i < safeView.length; i++) {
    newArray.push(safeView[i]);
  }
  return newArray;
}
