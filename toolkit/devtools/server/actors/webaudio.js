...
let WebAudioActor = exports.WebAudioActor = ActorClass({
  ...
  getMemory: method(Task.async(function *() {
    ...
    return Object.keys(mem).reduce((ret, nativeID) => {
      ...
      console.log(nativeID, mem[nativeID], this._nativeToActorID.get(~~nativeID));
      if (actor) {
      console.log(actor.actorID, mem[nativeID]);
  ...
  _constructAudioNode: function (node) {
    ...
    this.manage(actor);
    console.log("CONSTRUCTING", node.id, actor.actorID);
